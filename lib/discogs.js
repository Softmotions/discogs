(function() {
  ...
  exports = module.exports = function(format) {
    ...
    discogsRequest = function(url, next) {
      var rparse,
        _this = this;
      rparse = function(err, res, body) {
        var _ref;
        if (err) return next(err);
        if (((_ref = res.headers['content-type']) != null ? _ref.indexOf('json') : void 0) >= 0 || !format) {
          try {
      ...
      return mh.server.RemoteAPIBottleneck.getInstance().request("discogs", {
        uri: getUrl(url),
        headers: {
          'accept-encoding': 'gzip',
          'user-agent': sm.app.Env.getDefault().getUserAgent()
        },
        encoding: null,
        proxy: sm.app.Env.getDefault().getHTTPProxy()
      }, function(error, res, body) {
        var _ref, _ref2;
        if (!error && (200 <= (_ref = res.statusCode) && _ref < 400) && body) {
          if (((_ref2 = res.headers['content-encoding']) != null ? _ref2.indexOf('gzip') : void 0) >= 0) {
            zlib.gunzip(body, function(err, body) {
              return rparse(err, res, body);
            });
          } else {
